...
export function MainLayout({ children }: MainLayoutProps) {
  const { mode, setMode, settings, updateSettings, history, clearHistory, theme, setTheme } = useAppStore();

  return (
    <div className="flex flex-col md:flex-row w-full min-h-screen bg-gray-950 text-white font-sans overflow-hidden">
      {/* Sidebar / Settings Area */}
      <aside className="w-full md:w-80 lg:w-96 flex-shrink-0 bg-gray-900 border-r border-gray-800 flex flex-col h-screen overflow-y-auto">
        ...
        <div className="flex-1 p-6 flex flex-col gap-8">
          ...
          <div className="flex flex-col gap-4">
            ...
            >
              ...
              {settings.soundEnabled ? <Volume2 className="w-5 h-5 text-indigo-400" /> : <VolumeX className="w-5 h-5 text-gray-500" />}
            </button>
                ...
                <ul className="flex flex-col gap-2">
                  {history.map((h, i) => (
                    <li key={i} className="flex justify-between items-center text-sm border-b border-gray-700 pb-2 last:border-0">
                      ...
                      <span className="text-xs text-gray-500">{new Date(h.date).toLocaleTimeString()}</span>
                    </li>
                  ))}
                </ul>
              )}
      ...
      <main className="flex-1 flex flex-col relative h-screen z-10 bg-transparent">
        ...
        <header className="absolute top-0 left-0 right-0 p-4 flex justify-center z-50">
          <div className="bg-gray-900/80 backdrop-blur-md border border-gray-800 p-1 rounded-full flex gap-1 shadow-lg">
            <button
              ...
              className={clsx(
                ...
                mode === 'marquee' ? "bg-indigo-600 text-white shadow-md" : "text-gray-400 hover:text-white hover:bg-gray-800"
              )}
            >
              <LayoutGrid className="w-4 h-4" /> 跑馬燈
            </button>
            <button
              onClick={() => setMode('wheel')}
              className={clsx(
                "flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium transition-all",
                mode === 'wheel' ? "bg-indigo-600 text-white shadow-md" : "text-gray-400 hover:text-white hover:bg-gray-800"
              )}
            >
              <CircleDashed className="w-4 h-4" /> 轉盤
            </button>
            <button
              onClick={() => setMode('slot')}
              className={clsx(
                "flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium transition-all",
                mode === 'slot' ? "bg-indigo-600 text-white shadow-md" : "text-gray-400 hover:text-white hover:bg-gray-800"
              )}
            >
              <AlignEndHorizontal className="w-4 h-4" /> 拉霸
            </button>
            <button
              onClick={() => setMode('gacha')}
              className={clsx(
                "flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium transition-all",
                mode === 'gacha' ? "bg-indigo-600 text-white shadow-md" : "text-gray-400 hover:text-white hover:bg-gray-800"
              )}
            >
              <span className="text-base leading-none">✨</span> 抽卡
            </button>
          </div>
        </header>

        {/* Dynamic Game Mode Content */}
        <div className={clsx(
          "flex-1 flex items-center justify-center pt-20 pb-8 px-4 relative z-10",
          theme === 'default' && "bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-gray-900 via-gray-950 to-black"
        )}>
          {children}
        </div>
      </main>
